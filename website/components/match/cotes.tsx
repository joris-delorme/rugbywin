...
export function CardsMetric({team}: {team: string}) {
    const [cotes, setCotes] = useState<IBet[]>([])
    const { matches }= useMatches()

    useEffect(() => {
        if (team && matches.length && !cotes.length) {
            console.log(team)
            console.log(matches)
            const data = matches.filter(x => x.teams.team_a === team || x.teams.team_b === team)
            console.log(data)
            data.forEach((match) => {
                setCotes(old => [...old, {
                    team_a: team,
                    team_b: match.teams.team_a === team ? match.teams.team_b : match.teams.team_a,
